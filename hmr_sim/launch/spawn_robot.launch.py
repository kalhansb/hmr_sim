...
def namespace_sdf_file(sdf_path, params):
    ...
    for plugin in tree.findall('model/plugin'):
        ...
        if odom_topic is not None:
            plugin.find('odom_topic').text = namespace + '/' + odom_topic.text
        odom_covariance_topic = plugin.find('odom_covariance_topic')
        if odom_covariance_topic is not None:
            plugin.find('odom_covariance_topic').text = namespace + '/' + odom_covariance_topic.text
        tf_topic = plugin.find('tf_topic')
        if tf_topic is not None:
            plugin.find('tf_topic').text = namespace + '/' + tf_topic.text
        # Give a unique topic name for odom frame
        odom_frame = plugin.find('odom_frame')
        if odom_frame is not None and odom_frame.text == 'odom':
            plugin.find('odom_frame').text = namespace + '/odom'
        #####################################################################
        # For UAV:
...
def launch_setup(context):
    ...
    config_file = os.path.join(hmr_sim_share_dir, 'config', 'spawn_robot.yaml')
    ...
    sdf_path = os.path.join(hmr_sim_share_dir, 'models', params['sdf_file'])
